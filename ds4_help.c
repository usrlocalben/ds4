...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    if (full) {
        ...
        if (tool != DS4_HELP_SERVER) {
            opt(fp, c, "--cpu-moe", "Route routed MoE layers to CPU via kt-kernel bridge.");
            opt(fp, c, "--n-cpu-moe-layers N", "Number of MoE layers to offload (0 = all). Default: all");
            opt(fp, c, "--kt-weight-path PATH", "Path to kt-kernel safetensor weight directory.");
            opt(fp, c, "--kt-cpuinfer N", "kt-kernel CPU inference threads. Default: 96");
            opt(fp, c, "--kt-threadpool-count N", "kt-kernel NUMA thread pool count. Default: 8");
            opt(fp, c, "--kt-method NAME", "kt-kernel compute method: MXFP4 (default), FP8, FP8_PERCHANNEL, RAWINT4, AMXINT4, AMXINT8.");
        }
    }
    fputc('\n', fp);
}
