...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    if (graph_backend) {
        ...
        /* CPU-MoE flag must be set before model map setup so the
         * span-based registration path can use it to skip bulk
         * cudaHostRegister of the full model. */
        ...
        /* When --cpu-moe is active without --load-slice, build include spans
         * covering only non-expert tensors.  These are used both for the model
         * map setup (skipping the expensive bulk cudaHostRegister of the full
         * 150 GiB model) and for accelerator_cache_model_tensors below. */
        uint64_t *cpu_moe_offsets = NULL;
        uint64_t *cpu_moe_sizes = NULL;
        uint32_t cpu_moe_span_count = 0;
        if (e->cpu_moe && !load_slice) {
            ds4_byte_range expert_ranges[DS4_MAX_LAYER * 3];
            size_t n_expert = engine_collect_cpu_moe_routed_ranges(e, expert_ranges);
            cpu_moe_offsets = calloc((size_t)e->model.n_tensors, sizeof(uint64_t));
            cpu_moe_sizes = calloc((size_t)e->model.n_tensors, sizeof(uint64_t));
            for (uint64_t i = 0; i < e->model.n_tensors; i++) {
                const ds4_tensor *t = &e->model.tensors[i];
                if (t->bytes == 0) continue;
                bool is_expert = false;
                for (size_t s = 0; s < n_expert; s++) {
                    if (t->abs_offset < expert_ranges[s].end && t->abs_offset + t->bytes > expert_ranges[s].start) {
                        is_expert = true;
                        break;
                    }
                }
                if (!is_expert) {
                    cpu_moe_offsets[cpu_moe_span_count] = t->abs_offset;
                    cpu_moe_sizes[cpu_moe_span_count] = t->bytes;
                    cpu_moe_span_count++;
                }
            }
        }
        ...
        if (load_slice) {
            ...
            /* Use span-based setup to avoid bulk cudaHostRegister of the full
             * model.  Non-expert spans are small enough that per-range
             * registration on demand is fast and safe. */
            model_map_ok = ds4_gpu_set_model_map_spans(e->model.map,
                                                        e->model.size,
                                                        cpu_moe_offsets,
                                                        cpu_moe_sizes,
       ...
       if (e->mtp_ready &&
        ...
        /* Use pre-built cpu-moe spans when active, otherwise use load-slice spans
         * (or NULL/0 for all tensors when neither is active). */
        const uint64_t *use_offsets = cpu_moe_offsets ? cpu_moe_offsets : load_offsets;
        const uint64_t *use_sizes = cpu_moe_sizes ? cpu_moe_sizes : load_sizes;
        uint32_t use_count = cpu_moe_offsets ? cpu_moe_span_count : load_span_count;
        if (!accelerator_cache_model_tensors(e->backend, &e->model,
                                             use_offsets, use_sizes, use_count)) {
            free(cpu_moe_offsets);
            free(cpu_moe_sizes);
            free(load_offsets);
            free(load_sizes);
